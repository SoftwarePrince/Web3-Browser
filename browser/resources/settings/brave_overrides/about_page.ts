...
RegisterPolymerTemplateModifications({
  'settings-about-page': (templateContent) => {
    ...
    if (!section.querySelector('a#release-notes')) {
      ...
      wrapper.setAttribute('href', 'https://kahf.co/latest/')
    ...
    if (updateStatusMessageLink) {
      // <if expr="is_win">
      updateStatusMessageLink.href = 'https://support.kahf.co/hc/en-us/articles/360042816611-Why-isn-t-Brave-updating-automatically-on-Windows-'
      // </if>

      // <if expr="not is_win">
      updateStatusMessageLink.href = 'https://community.kahf.co?p=update_error'
      // </if>
    }
  }
})

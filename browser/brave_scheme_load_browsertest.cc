...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, NotAllowedToLoadTest) {
  ...
  console_observer.SetPattern(
      "Not allowed to load local resource: browseweb3://settings/");
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  ASSERT_TRUE(
      content::ExecJs(active_contents(), "window.open(\"browseweb3://settings\")"));
  ASSERT_TRUE(console_observer.Wait());
}
...
// This test is for the browseweb3:// scheme. This is a regression test added with:
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, MAYBE_CrashURLTest) {
  ...
  browser()->OpenURL(
      content::OpenURLParams(GURL("browseweb3://crash/"), content::Referrer(),
...
// Ex, browseweb3://settings should be loaded only in normal window because
// chrome://settings is not allowed. When tyring to loading browseweb3://settings in
// private window, it should be loaded in normal window instead of private
// window.
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       SettingsPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("browseweb3://settings");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("browseweb3://rewards");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("browseweb3://wallet");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       WalletPageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("browseweb3://wallet"));
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("browseweb3://sync");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("browseweb3://welcome");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       BraveWelcomePageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("browseweb3://welcome"));
}

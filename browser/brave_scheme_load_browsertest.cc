...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, NotAllowedToLoadTest) {
  ...
  console_observer.SetPattern(
      "Not allowed to load local resource: kahf://settings/");
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  ASSERT_TRUE(
      content::ExecJs(active_contents(), "window.open(\"kahf://settings\")"));
  ASSERT_TRUE(console_observer.Wait());
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest, MAYBE_CrashURLTest) {
  ...
  browser()->OpenURL(
      content::OpenURLParams(GURL("kahf://crash/"), content::Referrer(),
...
// Ex, kahf://settings should be loaded only in normal window because
// chrome://settings is not allowed. When tyring to loading kahf://settings in
// private window, it should be loaded in normal window instead of private
// window.
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       SettingsPageIsNotAllowedInPrivateWindow) {
  TestURLIsNotLoadedInPrivateWindow("kahf://settings");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("kahf://rewards");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("kahf://wallet");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       WalletPageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("kahf://wallet"));
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("kahf://sync");
}
...
IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
  ...
  TestURLIsNotLoadedInPrivateWindow("kahf://welcome");
}

IN_PROC_BROWSER_TEST_F(BraveSchemeLoadBrowserTest,
                       BraveWelcomePageIsNotAllowedInGuestWindow) {
  TestURLIsNotLoadedInGuestWindow(GURL("kahf://welcome"));
}

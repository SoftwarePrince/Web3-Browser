...
TEST(BraveWalletUtilsUnitTest, GetNetworkURLTest) {
  ...
  for (const auto& chain : GetAllKnownChains(&prefs, mojom::CoinType::ETH)) {
    ...
    if (base::EndsWith(rpc_url.host(), "browseweb3.com")) {
  ...
  EXPECT_EQ(GURL("https://mainnet-beta-solana.browseweb3.com/rpc"),
  ...
  EXPECT_EQ(
      GURL("https://bitcoin-mainnet.wallet.browseweb3.com/"),
...
TEST(BraveWalletUtilsUnitTest, GetNetworkURLForKnownChains) {
  ...
  for (const auto& chain : GetAllKnownChains(&prefs, mojom::CoinType::ETH)) {
    auto network_url =
        GetNetworkURL(&prefs, chain->chain_id, mojom::CoinType::ETH);
    EXPECT_EQ(base::EndsWith(network_url.host(), ".browseweb3.com"),
...
TEST(BraveWalletUtilsUnitTest, GetChain) {
  ...
  mojom::NetworkInfo sol_mainnet(
      ...
      {GURL("https://mainnet-beta-solana.browseweb3.com/rpc")}, "SOL", "Solana", 9,
  ...
  mojom::NetworkInfo btc_mainnet(
      ...
      {GURL("https://bitcoin-mainnet.wallet.browseweb3.com/")}, "BTC", "Bitcoin", 8,
...
TEST(BraveWalletUtilsUnitTest, CustomNetworkMatchesKnownNetwork) {
  ...
  EXPECT_EQ(
      GetNetworkURL(&prefs, mojom::kPolygonMainnetChainId, mojom::CoinType::ETH)
          .GetWithoutFilename(),
      GURL("https://mainnet-polygon.browseweb3.com/"));
...
TEST(BraveWalletUtilsUnitTest, eTLDPlusOne) {
  ...
  EXPECT_EQ("browseweb3.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://blog.browseweb3.com"))));
  EXPECT_EQ("browseweb3.com",
            eTLDPlusOne(url::Origin::Create(GURL("https://...browseweb3.com"))));
  EXPECT_EQ(
      "browseweb3.com",
      eTLDPlusOne(url::Origin::Create(GURL("https://a.b.c.d.browseweb3.com/1"))));
...
TEST(BraveWalletUtilsUnitTest, MakeOriginInfo) {
  auto origin_info =
      MakeOriginInfo(url::Origin::Create(GURL("https://blog.browseweb3.com:443")));
  EXPECT_EQ("https://blog.browseweb3.com", origin_info->origin_spec);
  EXPECT_EQ("browseweb3.com", origin_info->e_tld_plus_one);
...
TEST(BraveWalletUtilsUnitTest, GetUnstoppableDomainsRpcUrl) {
  EXPECT_EQ(AddInfuraProjectId(GURL("https://mainnet-infura.browseweb3.com")),
            GetUnstoppableDomainsRpcUrl(mojom::kMainnetChainId));
  EXPECT_EQ(AddInfuraProjectId(GURL("https://mainnet-polygon.browseweb3.com")),
...
TEST(BraveWalletUtilsUnitTest, GetSnsRpcUrl) {
  EXPECT_EQ(GURL("https://mainnet-beta-solana.browseweb3.com/rpc"), GetSnsRpcUrl());
}

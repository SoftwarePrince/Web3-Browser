...
def CheckLicense(input_api, output_api):
    """Verifies the BrowseWeb3 license header."""
    ...
    new_file_license_re = input_api.re.compile((
        r'.*? Copyright \(c\) %(year)s The BrowseWeb3 Authors\. All rights reserved\.\n'
    ...
    expected_license_template = (
        '%(comment)s Copyright (c) %(year)s The BrowseWeb3 Authors. All rights reserved.\n'
...
# Extend BanRule exclude lists with BrowseWeb3-specific paths.
...
def CheckForIncludeGuards(original_check, input_api, output_api, **kwargs):
    ...
    def AffectedSourceFiles(self, original_method, source_file):
        def PrependBrowseWeb3(affected_file):

...
def check_for_chromium_upgrade_extra_langs(src_root, grd_file_path):
    """Checks the BrowseWeb3 GRD file vs the Chromium GRD file for extra
       languages."""
    ...
    assert len(x_brave_extra_langs) == 0, \
        f'BrowseWeb3 GRD {grd_file_path} has extra languages ' \
    ...
    assert len(x_chromium_extra_langs) == 0, \
        f'Chromium GRD {chromium_grd_file_path} has extra languages ' \
            f'{list(x_chromium_extra_langs)} over BrowseWeb3 GRD {grd_file_path}'
...
def braveify(string_value):
    """Replace Chromium branded strings with BrowseWeb3 branded strings."""
    return (string_value.replace('Chrome', 'BrowseWeb3')
            .replace('Chromium', 'BrowseWeb3')
            .replace('Google', 'BrowseWeb3')
            .replace('BrowseWeb3 Docs', 'Google Docs')
            .replace('BrowseWeb3 Drive', 'Google Drive')
            .replace('BrowseWeb3 Play', 'Google Play')
            .replace('BrowseWeb3 Safe', 'Google Safe')
            .replace('Sends URLs of some pages you visit to BrowseWeb3',
                     'Sends URLs of some pages you visit to Google')
            .replace('BrowseWeb3 Account', 'BrowseWeb3 sync chain')
            .replace('BrowseWeb3 Lens', 'Google Lens')
            .replace('BrowseWeb3book', 'Chromebook')
            .replace('BrowseWeb3cast', 'Chromecast')
            .replace('BrowseWeb3 Cloud', 'Google Cloud')
            .replace('BrowseWeb3 Pay', 'Google Pay')
            .replace('BrowseWeb3 Photos', 'Google Photos')
            .replace('BrowseWeb3 Projects', 'Chromium Projects')
            .replace('BrowseWeb3 Widevine', 'Google Widevine'))
